...
func buildAdmissionGuardPlan(candidates []candidateAction) ProtectedWorkloadPlan {
	grouped := make(map[string][]candidateAction)
	for _, candidate := range candidates {
		grouped[candidate.WorkloadID] = append(grouped[candidate.WorkloadID], candidate)
	}
	...
	blocked := make([]blockedHead, 0, len(grouped))
	for workloadID, items := range grouped {
		immediateAny := false
		provisionableAny := false
		gangAny := false
		for _, item := range items {
			if item.ImmediateFit {
				immediateAny = true
			}
			if item.Provisionable {
				provisionableAny = true
			}
			if item.WorkerCount >= 2 && item.TotalGPU > item.PerWorkerGPU {
				gangAny = true
			}
		}
		if immediateAny || !provisionableAny || !gangAny {
			continue
		}
		slices.SortFunc(items, func(left, right candidateAction) int {
			switch {
			case left.Priority != right.Priority:
				...
				return 1
			case left.WaitSeconds != right.WaitSeconds:
				if left.WaitSeconds > right.WaitSeconds {
					return -1
				}
				return 1
			case left.WorkerCount != right.WorkerCount:
				if left.WorkerCount > right.WorkerCount {
					return -1
				}
				return 1
			case left.TotalGPU != right.TotalGPU:
				if left.TotalGPU > right.TotalGPU {
					return -1
				}
				return 1
		...
		blocked = append(blocked, blockedHead{workloadID: workloadID, candidate: items[0]})
	}
	if len(blocked) == 0 {
		return ProtectedWorkloadPlan{}
	}
	slices.SortFunc(blocked, func(left, right blockedHead) int {
		switch {
		case left.candidate.Priority != right.candidate.Priority:
			if left.candidate.Priority > right.candidate.Priority {
				return -1
			}
			return 1
		case left.candidate.WaitSeconds != right.candidate.WaitSeconds:
			if left.candidate.WaitSeconds > right.candidate.WaitSeconds {
				return -1
			}
			return 1
		case left.candidate.WorkerCount != right.candidate.WorkerCount:
			if left.candidate.WorkerCount > right.candidate.WorkerCount {
				return -1
			}
			return 1
		case left.candidate.TotalGPU != right.candidate.TotalGPU:
			if left.candidate.TotalGPU > right.candidate.TotalGPU {
				return -1
			}
			return 1
		case left.workloadID < right.workloadID:
			return -1
		case left.workloadID > right.workloadID:
			return 1
		default:
	...
	protected := blocked[0]
	protectedItems := grouped[protected.workloadID]
	flavors := make([]kueue.ResourceFlavorReference, 0, len(protectedItems))
	seen := make(map[kueue.ResourceFlavorReference]struct{}, len(protectedItems))
	for _, item := range protectedItems {
		flavor := kueue.ResourceFlavorReference(item.FlavorName)
		if _, ok := seen[flavor]; ok {
			continue
		}
		seen[flavor] = struct{}{}
		flavors = append(flavors, flavor)
	}
	slices.SortFunc(flavors, func(left, right kueue.ResourceFlavorReference) int {
		switch {
	...
	return ProtectedWorkloadPlan{
		ProtectedWorkload:    workload.Reference(protected.workloadID),
		ProtectedFlavors:     flavors,
		ProtectedPriority:    protected.candidate.Priority,
		ProtectedWorkerCount: protected.candidate.WorkerCount,
		ProtectedTotalGPU:    protected.candidate.TotalGPU,
	}
}
